    ...
    */
    ...
    if ( ! function_exists( 'sf_mobile_header' ) ) {
        function sf_mobile_header() {
			...
			if ( sf_theme_supports('hamburger-css') ) {
				...
				$mobile_menu_link = '<a href="#" class="mobile-menu-link menu-bars-link"><span class="button-text">' . __( 'Menu', 'uplift' ) . '</span>' . $mobile_menu_icon . '</a>';
			}
    ...
    if ( ! function_exists( 'sf_mobile_menu' ) ) {
        function sf_mobile_menu() {
            ...
            if ( $mobile_show_search ) {
                $mobile_menu_output .= '<form method="get" class="mobile-search-form" action="' . home_url() . '/"><label for="s" class="hidden-label">' . __( "Search", "swiftframework" ) . '</label><input type="text" placeholder="' . __( "Enter text to search", "swiftframework" ) . '" name="s" autocomplete="off" />';
    ...
    if ( ! function_exists( 'sf_get_search' ) ) {
        function sf_get_search( $type ) {
            ...
            $search_output .= '<li class="menu-search parent"><a href="#" class="header-search-link-alt">'.$search_icon.'<div class="hidden_screenreader">Search</div></a>' . "\n";
            ...
            $search_output .= '<label for="s" class="hidden-label">' . __( "Search", "swiftframework" ) . '</label><input type="text" placeholder="' . __( "Search", "swiftframework" ) . '" name="s" autocomplete="off" /></form><div class="ajax-search-results"></div></div>' . "\n";
            $search_output .= '</li>' . "\n";
            ...
            if ( sf_woocommerce_activated() ) {
                ...
                if ( $cart_count != "0" ) {
                    ...
                    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
                        ...
                        if ( !$_product || !method_exists( $_product, 'exists' ) ) {
                        	return;
                        }
            ...
            if ( count( $wishlist ) > 0 ) :
                ...
                foreach ( $wishlist as $values ) :
                    ...
                    if ( $count_limit < 3 ) {
						...
						$product_id = method_exists( $product_obj, 'get_id' ) ? $product_obj->get_id() : $product_obj->id;
