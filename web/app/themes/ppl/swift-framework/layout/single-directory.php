<?php
    ...
    $default_right_sidebar  = $sf_options['default_right_sidebar'];
     ...
     /* Default Directory Meta Options */ 
    $map_zoom = $sf_options['default_directory_item_map_zoom'];
    $map_type = $sf_options['default_directory_item_map_type'];
    $map_color = $sf_options['default_directory_item_map_color'];
    $map_saturation = $sf_options['default_directory_item_map_saturation'];
    $map_controls = $sf_options['default_directory_item_map_controls'];
    $map_fullscreen = $sf_options['default_directory_item_map_fullscreen'];
    ...
    if( sf_get_post_meta( $post->ID, 'sf_directory_map_pin', true ).length > 0 ){
        $map_pin = sf_get_post_meta( $post->ID, 'sf_directory_map_pin', true );
    } else {
        $map_pin = $sf_options['default_directory_map_pin']['id'];
    }
        ...
        <section class="page-content clearfix <?php echo esc_attr($page_content_class); ?>">
            ...
            <div class="content-wrap <?php echo esc_attr($content_wrap_class); ?> clearfix" itemprop="articleBody">
                ...
                <h3><?php _e( 'Location', 'swiftframework' )?></h3> 
                     ...
                     <?php echo do_shortcode('[spb_gmaps size="400" map_controls="' . $map_controls . '" type="' . $map_type . '" color="' . $map_color . '" zoom="' . $map_zoom . '" saturation="' . $map_saturation . '" fullscreen="' . $map_fullscreen . '" width="1/1" el_position="first last"] [spb_map_pin pin_title="' . get_the_title() . '" address="' . $map_address .'" pin_latitude="' . $map_lat_cord . '" pin_longitude="' . $map_lng_cord . '" pin_image="' . $map_pin . '" pin_link="' . $map_pin_link . '" pin_button_text="' . $map_pin_button_text . '" width="1/1" el_position="first last"][/spb_map_pin] [/spb_gmaps]'); ?>
